...
Serializable whole-application graph and impact queries for Frontier apps.

`frontier-application` turns packages, features, routes, views, actions, mutations, state paths, effects, workers, assets, tests, traces, policies, workflows, migrations, and benchmarks into one queryable graph.
...
```sh
npm install @shapeshift-labs/frontier-application
```
...
const app = compileApplicationGraph(createApplicationGraph({
  nodes: [{
...
`@shapeshift-labs/frontier-manifest` owns build/static package data. `@shapeshift-labs/frontier-inspect` owns runtime evidence bundles. `@shapeshift-labs/frontier-trace` owns traces. `@shapeshift-labs/frontier-test` owns declarative tests. `@shapeshift-labs/frontier-history` owns temporal explanation.

`@shapeshift-labs/frontier-application` sits above those surfaces as a structural graph. It can ingest compatible registry or manifest-like data, but the root package depends only on `@shapeshift-labs/frontier`.
...
The package benchmark writes `benchmarks/results/frontier-application-package-bench-latest.json` when run from the monorepo. The competitor/control benchmark writes `benchmarks/results/frontier-application-competitors-latest.json`. Frontier-only package measurements cover graph creation, compilation, route/state/feature queries, impact traversal, route action lookup, hot-path benchmark lookup, feature maps, registry graph output, JSONL, and proof hashing.
